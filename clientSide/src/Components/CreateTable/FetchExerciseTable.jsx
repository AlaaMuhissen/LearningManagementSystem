...
import EditExerciseInfo from '../EditForm/EditExerciseInfo';
import { Button, CircularProgress } from '@mui/material';
import { useNavigate } from 'react-router-dom';

function FetchExerciseTable() {
  const auth = getAuth();
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const retryResponse = await fetch('https://learningmanagementsystem.onrender.com/api/exercises/getAllExercises', {
          headers: {
        ...
        if (retryResponse.ok) {
          ...
          console.error('Failed to fetch exercises after token refresh:', retryResponse.statusText);
        }
      } else {
  ...
  const handleDeleteClick = async (row) => {
    try {
      const freshToken = await refreshIdToken();
      const response = await fetch(`https://learningmanagementsystem.onrender.com/api/exercises/deleteExercise/${row.id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${freshToken}`,
          'Content-Type': 'application/json',
    ...
    <>
      ...
      {!loading && rows.length !== 0 && columns.length !== 0 && (
        <>
          ...
          {editRow && <EditExerciseInfo row={editRow} onClose={() => setEditRow(null)} />}
        </>
      )}
      <ToastContainer />
      <Button
...
export default FetchExerciseTable;
