...
export const LanguageStatusProvider = ({ children }) => {
  ...
  const fetchProgressData = async (syllabus_id, language_id) => {
    try {
      const response = await fetch(`${API_URL}/api/progress/getLanguageStatus/${userId}/syllabus/${syllabus_id}/language/${language_id}`);
  ...
  const updateLanguageStatus = async (started, finished) => {
    try {
      const response = await fetch(`${API_URL}/api/progress/updateLanguageStatus/${userId}`, {
        method: 'PUT',
        headers: {
